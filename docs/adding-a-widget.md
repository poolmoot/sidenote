...
5. **Add an id**: a `public static let` constant on `NotchWidgetAPI.WidgetID` (e.g. `public static let <name> = WidgetID(rawValue: "<name>")`) — see the note at the end. Also add it to `Preferences.knownWidgetIDs` in `SettingsFeature`, so the Widgets and Shortcuts tabs pick it up and a saved list from before it existed appends it rather than dropping it.
...
Delete the module folder and its tests, its product/target in `Package.swift`, its line in `project.yml`, its two lines in `AppEnvironment`, its `WidgetID` constant, and its entry in `Preferences.knownWidgetIDs`. Nothing else in the codebase names it. Its saved JSON file stays on disk harmlessly until deleted, and any shortcut assignment for it is silently ignored (`NotchStateMachine` only reacts to a `.shortcut(id)` for a widget it still holds).
